...
function isResponseComplete(content: string): boolean {
  ...
  const openLinks = (trimmedContent.match(/\[/g) || []).length;
  const closeLinks = (trimmedContent.match(/\]/g) || []).length;
  if (openLinks !== closeLinks) {
    return false;
  }
  const urlMatches = trimmedContent.match(/\(https?:\/\/[^\)]*/g) || [];
  const incompleteUrls = urlMatches.filter(url => !url.endsWith(')'));
  if (incompleteUrls.length > 0) {
    return false;
  }
  const lastChars = trimmedContent.slice(-10);
  const hasProperEnding = /[.!?)]\s*$/.test(trimmedContent) || /\]\(https?:\/\/[^\)]+\)\s*$/.test(trimmedContent);
  return hasProperEnding || trimmedContent.length < 50;
}

// Function to check if response is complete - improved version
function isResponseCompleteV2(content: string): boolean {
  const trimmed = content.trim();
  if (trimmed.length < 10) return false;
  const lastChar = trimmed[trimmed.length - 1];
  const secondLastChar = trimmed.length > 1 ? trimmed[trimmed.length - 2] : '';
  return lastChar === '.' || lastChar === '!' || lastChar === '?' || lastChar === ')' ||
         (lastChar === 's' && secondLastChar === 's');
}
...
async function validateUrl(url: string): Promise<boolean> {
  try {
    ...
    if (domain === 'localhost' || domain === '127.0.0.1' || domain.startsWith('192.168') || domain.startsWith('10.')) {
      return true;
    }
...
function extractSourcesFromResponse(content: string): Array<{ title: string; url: string }> {
  const sources: Array<{ title: string; url: string }> = [];
  const sourceSectionMatch = content.match(/(?:Sumber:|Sources:|Referensi:)\s*\n([\s\S]*?)(?=\n\n|\n*$)/i);
  if (sourceSectionMatch) {
    const sourceSection = sourceSectionMatch[1];
    const linkMatches = sourceSection.match(/\[([^\]]+)\]\(([^)]+)\)/g) || [];
  ...
  for (const line of lines) {
    ...
    if (linkMatch) {
      ...
      if (!sources.some(s => s.url === url)) {
        sources.push({ title: linkMatch[1], url });
      }
    }
  }
  return sources;
...
async function validateSources(content: string): Promise<{ isValid: boolean; invalidSources: string[]; validSources: string[] }> {
  const sources = extractSourcesFromResponse(content);
  ...
  for (const source of sources) {
    ...
    if (isValid) {
      ...
      invalidSources.push(source.url);
    }
  }
  return {
